"""Orquestracao do motor de capitalizacao: compra em pacotes, planos,
campanhas e o webhook de confirmacao de pagamento.
...
Dominios vizinhos: numeros da sorte/apuracao vivem em `sorteios/`,
recorrencia no cartao em `assinaturas/` e o gateway em `pagamentos/`.
"""
...
from baita_coin.sorteios import repository as sorteios_repo
from baita_coin.sorteios.errors import NenhumSorteioAberto
...
def _montar_detalhe(conn, compra: Row) -> CompraDetalheResponse:
    ...
    numeros = sorteios_repo.get_numeros_sorte_por_evento(conn, compra.event_id)
    if numeros:
